...
import { PencilIcon, XIcon, CheckCircleIcon } from "@/components/icons";
...
export default function EditClientModal({ client }: Props) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !showSuccess) setIsOpen(false);
    };
          ...
          <div className="relative w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl p-6 sm:p-8 shadow-xl animate-slide-up">
            {showSuccess ? (
              <div className="flex flex-col items-center justify-center py-10 animate-fade-in">
                ...
                <p className="text-lg font-semibold text-sage-800">Client Updated</p>
                <p className="text-sm text-sage-500 mt-1">Changes saved successfully.</p>
              </div>
            ) : (
              <>
